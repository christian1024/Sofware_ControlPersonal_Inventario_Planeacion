...
@section('contenidoPrincipal')
    ...
    <div class="card row">
                ...
                <div class="form-group row card-body">
                    <div class="col-4">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <label>{{__('Tipo Resultado')}}</label>
                                <select class="labelform form-control" required id="TipoResultado" name="TipoResultado">
                    ...
                    <div class="col-8">
                        <div id="divIndex" style="display: none" class="card">
                            <form id="IndexMasivo" action="{{ route('ResultadoIndexMasivo') }}" method="post">
                                @csrf
                                <input type="hidden" name="Identificador" value="{{ $IdentificadorC }}"/>
                                <div class="card-header text-center">
                                    Resultado Masivo Index
                                </div>
                                <div class="card-body text-center">
                                    ...
                                    <div class="col-12">
                                        <button type="submit" class="btn btn-block btn-outline-success">Guardar Resultado Masivo</button>
                                    </div>
                                </div>
                            </form>
                        </div>

                        <div id="divScreen" style="display: none" class="row card">
                            <form id="ScreenMasivo" action="{{ route('ResultadoScreenMasivo') }}" method="post">
                                @csrf

                                <input type="hidden" name="Identificador" value="{{ $IdentificadorC }}"/>
                                <div class="card-header text-center">
                                    Resultado Masivo Screen
                                </div>
                                <div class="row col-12 card-body">
                                    <div class="col-4">
                                        <label>Muestras Realizadas</label>
                                        <br>
                                        <select class="selectpicker" data-show-subtext="true" name="MuestraRealizadasMa[]" id="MuestraRealizadasMa" data-live-search="true" multiple data-actions-box="true" data-selected-text-format="count > 10" required="required">
                                    ...
                                    <div class="col-4">
                                        ...
                                        <br>
                                        <select class="selectpicker" data-show-subtext="true" name="MuestraRealizadasPositivasMa[]" id="MuestraRealizadasPositivasMa" data-live-search="true" multiple data-actions-box="true" data-selected-text-format="count > 10">
                                            @foreach( $virus as $viru)
                                                <option value=" {{ $viru->Siglas }}"> {{ $viru->NombreVirus }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-4">
                                        ...
                                        <select class="selectpicker" data-show-subtext="true" name="MuestraRealizadasPositivasRestringidosMa[]" id="MuestraRealizadasPositivasRestringidosMa" data-live-search="true" multiple data-actions-box="true" data-selected-text-format="count > 10">
        ...
        <div class="container-fluid card-body col-lg-12">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#MuestraRealizadasPositivasMa').change(function () {
                let Muestra = $('#MuestraRealizadasPositivasMa').val();
                console.log(Muestra);
                $.ajax({
                    headers: {'X-CSRF-TOKEN': token},
                    data: {Muestra},
                    ...
                    success: function (Result) {
                        //  console.log(Result);
                        $("#MuestraRealizadasPositivasRestringidosMa").empty().selectpicker('destroy');
                        $("#MuestraRealizadasPositivasRestringidosMa").append('<option selected="true" value="" disabled="disabled">Seleccione uno...</option>');
                        $.each(Result.Data, function (i, item) {
                            $("#MuestraRealizadasPositivasRestringidosMa").append('<option value="' + item.Siglas + '">' + item.NombreVirus + '</option>');
                        });
                        $('#MuestraRealizadasPositivasRestringidosMa').selectpicker({
                            size: 4,
                            liveSearch: true,
                        });
                    },
            ...
            $('#TipoResultado').change(function () {
                let TipoMasivo = $('#TipoResultado').val();
